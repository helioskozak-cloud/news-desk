"""
Render free-tier worker that runs the news-desk refresh loop and pushes
fresh headlines.json / stocks.json back to GitHub on a 5-minute cadence.

Exposes / and /health (200 OK) so an external keep-alive pinger can keep
the dyno awake. Free Render web services sleep after 15 min of no
incoming HTTP traffic.

Run one gunicorn worker only — the refresh loop starts in a daemon
thread at import time, and N gunicorn workers would mean N concurrent
git pushes racing each other.
"""
import os
import sys
import time
...
def _configure_git():
    ...
    _run(["git", "config", "user.name", "news-desk-render-worker"])
    _run(["git", "config", "user.email", "worker@news-desk"])
    auth_url = f"https://x-access-token:{GH_PAT}@github.com/{GH_OWNER}/{REPO_NAME}.git"
    r = _run(["git", "remote", "set-url", "origin", auth_url])
    if r.returncode != 0:
        raise RuntimeError(f"remote set-url failed: {r.stderr[-400:]}")
...
def _refresh_loop():
    try:
        ...
        _status["last_error"] = f"git config: {e}"
        print(f"git config failed: {e}", flush=True)
        return
